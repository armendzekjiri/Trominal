...
export function buildSnippetPrompt(description: string): string {
  return [
    ...
    '',
    'Reply with strict JSON only (no Markdown, no commentary). Schema:',
    '{ "title": string, "body": string, "tags": string[] }',
    '',
    'The body should be a single shell command or short script. Use {{name}}',
    'placeholders for runtime variables (matched by Trominal). Tags are short',
    'lowercase keywords like "k8s" or "ops".',
    '',
    `Request: ${description}`,
  ].join('\n')
}
