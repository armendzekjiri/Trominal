...
import type { LocalShellOptions, SshSession, SshSessionState, Unsubscribe } from './types'
...
type LocalShellClosed = {
  session_id: string
  reason: string
}

export class LocalShellSession implements SshSession {
  readonly id: string
  ...
  constructor(private readonly options: LocalShellOptions = {}) {
  ...
  async connect(): Promise<void> {
    ...
    this.unlistenData = await listen<LocalShellData>('local-shell://data', (event) => {
      if (event.payload.session_id === this.nativeSessionId) {
        this.emitter.emitData(new Uint8Array(event.payload.data))
      }
    })
    this.unlistenClosed = await listen<LocalShellClosed>('local-shell://closed', (event) => {
      if (event.payload.session_id === this.nativeSessionId) {
    ...
    try {
      await invoke<string>('local_shell_open', {
        sessionId: this.id,
  ...
  async close(): Promise<void> {
    if (this.nativeSessionId !== null) {
      ...
      await invoke('local_shell_close', { sessionId: this.nativeSessionId })
    }
    this.nativeSessionId = null
    this.currentState = 'closed'
    this.cleanupListeners()
